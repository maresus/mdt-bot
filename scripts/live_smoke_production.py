...
def _post_chat(base_url: str, clinic_id: str, session_id: str, message: str) -> str:
    res = requests.post(
...
def _run_case(base_url: str, clinic_id: str, name: str, turns: list[str], expect_any: Iterable[str]) -> tuple[bool, str]:
    session_id = f"live-smoke::{name}::{uuid.uuid4().hex[:8]}"
    last_reply = ""
    for msg in turns:
        last_reply = _post_chat(base_url, clinic_id, session_id, msg).lower()
...
def main() -> int:
    ...
    print(json.dumps({"ok": ok, "results": summary}, ensure_ascii=False, indent=2))
