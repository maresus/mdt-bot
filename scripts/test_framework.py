...
class TestRunner:
    ...
    def test(self, case: dict[str, Any], idx: int) -> None:
        ...
        expected_min_conf = case.get("min_confidence")
        ...
        conf_ok = True
        if isinstance(expected_min_conf, (int, float)):
            conf_ok = float(decision.confidence) >= float(expected_min_conf)
        ok = intent_ok and service_ok and action_ok and conf_ok
        ...
        details = (
            ...
            f"  got=intent:{decision.primary_intent.value} service:{decision.service_type} action:{decision.action.value} confidence:{decision.confidence:.2f}"
        )
        if isinstance(expected_min_conf, (int, float)):
